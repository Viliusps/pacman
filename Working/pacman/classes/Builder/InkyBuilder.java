...
public class InkyBuilder {
	...
	public Inky startFastInky() {
		return new InkyConfigurator(new FastInky()).addX(96).addY(96).addDx(-1).addDy(0).addSpeed(4).addColor(blueColor).build();
	}
	
	public Inky startSlowInky() {
		return new InkyConfigurator(new SlowInky()).addX(96).addY(96).addDx(-1).addDy(0).addSpeed(2).addColor(blueColor).build();
	}
}
